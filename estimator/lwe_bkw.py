...
class CodedBKW:
    ...
    def __call__(self, params: LWEParameters, ntest=None):
        ...
        EXAMPLE::
            ...
            >>> params = LWEParameters(n=64, q=4099, Xs=ND.DiscreteGaussian(5.678), Xe=ND.DiscreteGaussian(5.678))
            >>> coded_bkw(params)
            rop: ≈2^48.8, m: ≈2^39.6, mem: ≈2^39.6, b: 3, t1: 2, t2: 10, ℓ: 2, #cod: 53, #top: 0, #test: 6
        ...
        if best["m"] > params.m:
            raise InsufficientSamplesError(f"m={params.m} < {best['m']} (required)")
        return best
