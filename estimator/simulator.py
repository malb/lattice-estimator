...
from sage.all import RR, log, line, cached_function, pi, exp, sqrt, floor
...
def ZGSA(d, n, q, beta, xi=1, tau=1, dual=False):
    ...
    for i in range(d):
        ...
        # try:
        #     L_log[(d - n)-i-1] = logq/2. + diff
        #     L_log[(d - n)+i  ] = logq/2. - diff
...
def ZGSA2(d, n, q, beta, xi=1, tau=1, dual=False):
    from math import lgamma
    """
    Reduced lattice Z-shape following the Geometric Series Assumption as specified in
    NTRU fatrigue [DucWoe21]
    ...
    def slope(beta):
        ...
        if beta<=70:
            # interpolate between experimental and asymptotics
            ratio = (70-beta)/10.
            return ratio*small_slope_t8[60]+(1.-ratio)*2*log(deltaf(70))
        else:
            return 2 * log(deltaf(beta))

    
    delta = exp(slope(beta) / 2)
    k = RR(1/2 + log(q)/(2*log(delta)))
    # Solve for the number of remaining q vectors and calculate volume
    if tau is None:
        log_vol = RR(log(q, 2) * (d - n) + log(xi, 2) * n)
    ...
    r_log = [(d - 1 - 2 * (i)) * RR(log(delta, 2)) + log_vol / d if i > (n - k) else RR(log(q, 2)) for i in range(d)]
    # floor basis at 0
    r_log = [r_log[i] if i < (n+k-1) else 0 for i in range(d)]
    # ceiling basis at log q
    # r_log = [r_ if r_ < RR(log(q, 2)) else RR(log(q, 2)) for r_ in r_log]
    r = [2 ** (2 * r_) for r_ in r_log]
    return r

def ZGSA3(d, n, q, beta, xi=1, tau=1, dual=False):
    """
    Reduced lattice shape following the Geometric Series Assumption [Schnorr03]_
    ...
    # Solve for the number of remaining q vectors and calculate volume
    if tau is None:
        k = min(floor(sqrt(n / log(delta, q))), d)
        # assert k >= n
        log_vol = RR(log(q, 2) * (k - n) + log(xi, 2) * n)
    else:
        k = min(floor(sqrt((n + 1) / log(delta, q))), d)
        # assert k >= n
        log_vol = RR(log(q, 2) * (k - n - 1) + log(xi, 2) * n + log(tau, 2))

    r_log = [(k - 1 - 2 * (i - (d - k))) * RR(log(delta, 2)) + log_vol / d if i >= (d - k) else RR(log(q, 2)) for i in range(d)]
    # floor basis at 0
    r_log = [r_ if r_ > 0 else 0 for r_ in r_log]
    r = [2 ** (2 * r_) for r_ in r_log]
    return r
